...
{#if guideOpen}
  ...
  '&lt;API_BASE_URL&gt;/api/version'</pre>
      ...
      <div class="security-note">Token 属于敏感凭据，请仅通过管理员提供的 API Base URL 使用，切勿泄露或发送到其他地址。</div>
...
<style>
  ...
  .hint{margin:6px 0 0;color:var(--text-muted);font-size:var(--font-size-sm)}button,input,select{box-sizing:border-box;border:1px solid var(--border-color);border-radius:5px;background:var(--bg-primary);color:var(--text-primary);padding:7px 10px}button{cursor:pointer}button:disabled{cursor:not-allowed;opacity:.5}.primary{border-color:var(--accent-blue-emphasis);background:var(--accent-blue-emphasis);color:var(--text-on-accent)}.danger{border-color:var(--accent-red);color:var(--accent-red)}
