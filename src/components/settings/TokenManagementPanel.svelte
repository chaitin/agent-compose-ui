<script lang="ts">
  ...
  async function createToken(): Promise<void> {
    ...
    error = '';
    try {
      created = await apiTokens.create(normalized, role);
      createOpen = false;
      copied = false;
      await load();
    } catch (cause) {
...
<section class="panel" aria-labelledby="token-title">
  ...
  {:else}
    ...
    <div class="table-wrap">
      <table>
        <thead><tr><th>名称</th><th>角色</th><th>创建时间</th><th>状态</th><th><span class="sr-only">操作</span></th></tr></thead>
        <tbody>
          {#each items as item (item.id)}
            <tr>
              ...
              <td>{#if item.revokedAt}<span class="revoked">已撤销 · {formatTime(item.revokedAt)}</span>{:else}<span class="active">有效</span>{/if}</td>
              <td><button onclick={() => revokeTarget = item} disabled={Boolean(item.revokedAt)}>撤销</button></td>
            </tr>
          {:else}
            <tr><td colspan="5" class="empty">尚未创建 API Token。</td></tr>
          {/each}
        </tbody>
      </table>
    </div>
