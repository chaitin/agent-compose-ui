<script lang="ts">
  ...
  function listFailure(value: unknown): { availability: Availability; message: string } {
    if (value instanceof VolumeFileApiError && value.status === 503 && value.code === 'unavailable') return { availability: 'unavailable', message: '卷文件服务未配置，配置后可重新加载。' };
    if (value instanceof VolumeFileApiError && value.status === 404 && value.code === 'not_found') return { availability: 'missing', message: '卷尚未创建，请先应用项目后重新加载。' };
  ...
  $effect(() => { const next = `${projectKey}\0${volume}`; if (next === identity) return; identity = next; for (const request of mutationControllers) request.abort(); path = ''; entries = []; preview = null; draft = ''; conflict = false; saving = false; mutationBusy = false; availability = 'pending'; deletion = null; void load(''); return () => { controller?.abort(); for (const request of mutationControllers) request.abort(); }; });
</script>

<section class="browser" aria-label={`卷文件 ${volume}`}>
  <header class="toolbar" role="toolbar" aria-label="卷文件操作"><strong>文件 · {volume}</strong><button class="ui-button ghost" type="button" disabled={mutationBusy || loading} onclick={() => requestLoad(path)}>重新加载</button><form onsubmit={(e) => { e.preventDefault(); createFolder(); }}><label>新建文件夹 <input disabled={mutationsDisabled} bind:value={folderName} aria-label="新建文件夹" /></label><button class="ui-button secondary" disabled={mutationsDisabled}>创建文件夹</button></form><label>上传文件 <input disabled={mutationsDisabled} type="file" aria-label="上传文件" onchange={upload} /></label></header>
  {#if error}<p role="alert">{error}</p>{/if}{#if loading}<p role="status">正在加载…</p>{/if}
  <div class="panes">
    <nav class="files" aria-label="卷文件列表"><div class="path"><button class="ui-button ghost" type="button" disabled={!path || mutationBusy || volumeUnavailable} onclick={() => requestLoad(path.split('/').slice(0,-1).join('/'))}>上一级</button><code>{path || '/'}</code></div>{#if !loading && availability === 'ready' && entries.length === 0}<p>目录为空</p>{/if}<ul>{#each entries as item}<li><button class="ui-button ghost" type="button" disabled={mutationBusy || volumeUnavailable} onclick={() => open(item)}>{item.name}</button><span>{item.dir ? '文件夹' : `${item.size} B`}</span><button class="ui-button danger" type="button" disabled={mutationsDisabled} aria-label={`删除 ${item.name}`} onclick={() => deletion = item}>删除</button></li>{/each}</ul></nav>
