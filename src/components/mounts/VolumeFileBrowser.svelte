...
<section class="browser" aria-label={`卷文件 ${volume}`}>
  <header><strong>文件 · {volume}</strong><button type="button" disabled={mutationBusy} onclick={() => requestLoad(path)}>重新加载</button></header>
  {#if error}<p role="alert">{error}</p>{/if}{#if loading}<p role="status">正在加载…</p>{/if}
  <nav aria-label="当前目录"><button type="button" disabled={!path || mutationBusy} onclick={() => requestLoad(path.split('/').slice(0,-1).join('/'))}>上一级</button><code>{path || '/'}</code></nav>
  <form onsubmit={(e) => { e.preventDefault(); createFolder(); }}><label>新建文件夹 <input disabled={mutationBusy || dirty} bind:value={folderName} aria-label="新建文件夹" /></label><button disabled={mutationBusy || dirty}>创建文件夹</button></form>
  <label>上传文件 <input disabled={mutationBusy || dirty} type="file" aria-label="上传文件" onchange={upload} /></label>
  {#if !loading && entries.length === 0}<p>目录为空</p>{/if}
  <ul>{#each entries as item}<li><button type="button" disabled={mutationBusy} onclick={() => open(item)}>{item.name}</button><span>{item.dir ? '文件夹' : `${item.size} B`}</span><button type="button" disabled={mutationBusy || dirty} aria-label={`删除 ${item.name}`} onclick={() => deletion = item}>删除</button></li>{/each}</ul>
  {#if preview}<section><h4>{preview.path}</h4><textarea disabled={saving} readonly={preview.truncated} aria-label="文件内容" bind:value={draft}></textarea>{#if preview.truncated}<p>仅显示部分内容，不能直接保存</p>{:else}<button type="button" disabled={mutationBusy || draft === preview.content} onclick={save}>{saving ? '正在保存…' : '保存文件'}</button>{#if conflict}<button type="button" disabled={mutationBusy} onclick={reloadPreview}>重新加载文件</button>{/if}{/if}<a href={volumeFileApi.downloadURL(projectKey, volume, preview.path)}>下载</a></section>{/if}
</section>
{#if deletion}<div class="backdrop"><div role="dialog" aria-modal="true" aria-label="确认删除"><p>确认删除 {deletion.path}？{deletion.dir ? '文件夹将递归删除。' : ''}</p><button type="button" onclick={() => deletion = null}>取消</button><button type="button" onclick={confirmDelete}>确认删除</button></div></div>{/if}

<style>.browser{border:1px solid var(--border-color);padding:8px;display:grid;gap:8px;width:100%}header,nav,li{display:flex;gap:8px;align-items:center}header strong{margin-right:auto}ul{margin:0;padding-left:20px}textarea{width:100%;min-height:120px}.backdrop{position:fixed;inset:0;background:#0008;display:grid;place-items:center;z-index:110}.backdrop>div{background:var(--bg-primary);padding:20px;border:1px solid var(--border-color)}</style>
