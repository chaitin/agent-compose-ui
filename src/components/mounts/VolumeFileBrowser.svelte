...
<section class="browser" aria-label={`卷文件 ${volume}`}>
  <header class="toolbar" role="toolbar" aria-label="卷文件操作"><strong>文件 · {volume}</strong><button type="button" disabled={mutationBusy} onclick={() => requestLoad(path)}>重新加载</button><form onsubmit={(e) => { e.preventDefault(); createFolder(); }}><label>新建文件夹 <input disabled={mutationBusy || dirty} bind:value={folderName} aria-label="新建文件夹" /></label><button disabled={mutationBusy || dirty}>创建文件夹</button></form><label>上传文件 <input disabled={mutationBusy || dirty} type="file" aria-label="上传文件" onchange={upload} /></label></header>
  {#if error}<p role="alert">{error}</p>{/if}{#if loading}<p role="status">正在加载…</p>{/if}
  <div class="panes">
    <nav class="files" aria-label="卷文件列表"><div class="path"><button type="button" disabled={!path || mutationBusy} onclick={() => requestLoad(path.split('/').slice(0,-1).join('/'))}>上一级</button><code>{path || '/'}</code></div>{#if !loading && entries.length === 0}<p>目录为空</p>{/if}<ul>{#each entries as item}<li><button type="button" disabled={mutationBusy} onclick={() => open(item)}>{item.name}</button><span>{item.dir ? '文件夹' : `${item.size} B`}</span><button type="button" disabled={mutationBusy || dirty} aria-label={`删除 ${item.name}`} onclick={() => deletion = item}>删除</button></li>{/each}</ul></nav>
    <section class="preview" aria-label="卷文件预览">{#if preview}<h4>{preview.path}</h4><textarea disabled={saving} readonly={preview.truncated} aria-label="文件内容" bind:value={draft}></textarea>{#if preview.truncated}<p>仅显示部分内容，不能直接保存</p>{:else}<button type="button" disabled={mutationBusy || draft === preview.content} onclick={save}>{saving ? '正在保存…' : '保存文件'}</button>{#if conflict}<button type="button" disabled={mutationBusy} onclick={reloadPreview}>重新加载文件</button>{/if}{/if}<a href={volumeFileApi.downloadURL(projectKey, volume, preview.path)}>下载</a>{:else}<p>请选择文件预览或编辑。</p>{/if}</section>
  </div>
</section>
