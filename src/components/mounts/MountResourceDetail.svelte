...
{#if resource}
  <section class="detail" aria-label={`${resource.key} 详情`}>
    ...
    {#each resource.mounts as mount}<div class="mount"><span>{mount.agent} → <code>{mount.target}</code> · {mount.readOnly ? '只读' : '读写'}</span><button class="ui-button secondary" type="button" disabled={locked} onclick={() => onUnmount(mount)}>卸载</button></div>{/each}
    <button class="ui-button danger" type="button" disabled={locked} onclick={(event) => onRemove(resource, event.currentTarget)}>移除声明…</button>
    {#if canBrowse}<div data-volume-browser></div>{/if}
  </section>
{:else if share}
  <section class="detail" aria-label={`${share.source} 详情`}><header><h3>共享目录</h3><code>{share.source}</code></header><p>{share.agent} → <code>{share.target}</code> · {share.readOnly ? '只读' : '读写'}</p><button class="ui-button secondary" type="button" disabled={locked} onclick={() => onUnmount(share)}>卸载</button></section>
