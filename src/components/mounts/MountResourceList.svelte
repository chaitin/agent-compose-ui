...
<nav aria-label="数据资源">
  <fieldset><legend>持久数据</legend>{#if !data.length}<p>暂无</p>{/if}{#each data as item}<button type="button" aria-current={active({ kind: 'volume', key: item.key }) ? 'true' : undefined} onclick={() => onSelect({ kind: 'volume', key: item.key })}>{item.key} <span>{item.mounts.length}</span></button>{/each}</fieldset>
  <fieldset><legend>依赖缓存</legend>{#if !caches.length}<p>暂无</p>{/if}{#each caches as item}<button type="button" aria-current={active({ kind: 'volume', key: item.key }) ? 'true' : undefined} onclick={() => onSelect({ kind: 'volume', key: item.key })}>{item.key} <span>{item.mounts.length}</span></button>{/each}</fieldset>
  <fieldset><legend>共享目录</legend>{#if !shares.length}<p>暂无</p>{/if}{#each shares as item}<button type="button" aria-current={active({ kind: 'share', source: item.source, agent: item.agent, target: item.target }) ? 'true' : undefined} onclick={() => onSelect({ kind: 'share', source: item.source, agent: item.agent, target: item.target })}>{item.source} · {item.agent}</button>{/each}</fieldset>
</nav>
<style>
