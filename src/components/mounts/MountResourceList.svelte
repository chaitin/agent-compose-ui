...
<nav aria-label="数据资源">
  ...
  <fieldset class="hidden-section"><legend>依赖缓存</legend>{#if !caches.length}<p>暂无</p>{/if}{#each caches as item}<button class="ui-button ghost" type="button" aria-current={active({ kind: 'volume', key: item.key }) ? 'true' : undefined} onclick={() => onSelect({ kind: 'volume', key: item.key })}>{item.key} <span>{item.mounts.length}</span></button>{/each}</fieldset>
  <fieldset class="hidden-section"><legend>共享目录</legend>{#if !shares.length}<p>暂无</p>{/if}{#each shares as item}<button class="ui-button ghost" type="button" aria-current={active({ kind: 'share', source: item.source, agent: item.agent, target: item.target }) ? 'true' : undefined} onclick={() => onSelect({ kind: 'share', source: item.source, agent: item.agent, target: item.target })}>{item.source} · {item.agent}</button>{/each}</fieldset>
</nav>
<style>
  nav { padding: 8px; border-right: 1px solid var(--border-color); overflow: auto; } fieldset { display: grid; gap: 4px; margin: 0 0 10px; border: 0; } fieldset.hidden-section { display: none; } legend { font-weight: 600; } button { display: flex; justify-content: space-between; text-align: left; } button[aria-current='true'] { color: var(--accent-blue); } p { margin: 2px 0; color: var(--text-secondary); }
</style>
