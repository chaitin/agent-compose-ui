<script lang="ts">
  ...
  function statusClass(status = execution?.status) {
    return ({ running: 'running', succeeded: 'success', failed: 'failed', canceled: 'canceled', skipped: 'skipped', unknown: 'unknown' } as Record<string, string>)[status || 'unknown'];
  }
