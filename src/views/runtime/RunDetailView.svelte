...
<style>
  ...
  .status-pill { display:inline-flex; align-items:center; gap:5px; padding:2px 6px; border:1px solid var(--border-color); border-radius:9px; color:var(--text-muted); font-size:var(--font-size-xs); font-weight:700; }.status-pill i { width:5px; height:5px; border-radius:50%; background:currentColor; }.status-pill.success { color:var(--accent-green); }.status-pill.running { color:var(--accent-green); }.status-pill.failed { color:var(--accent-red); }.status-pill.canceled { color:var(--accent-orange); }.status-pill.pending, .status-pill.unknown { color:var(--accent-yellow); }
