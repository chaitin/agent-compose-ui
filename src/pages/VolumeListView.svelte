...
<div class="root"><header><div><div class="scope">daemon 资源</div><h1>数据卷</h1><p>这里仅创建和管理 daemon 全局资源，不会修改项目 YAML，也不进入 Runtime。</p></div><div class="actions"><button onclick={load} disabled={loading}>刷新</button><button class="danger" onclick={() => prune(false)} disabled={operating}>预览清理</button></div></header>
  ...
  {#if error}<section class="state error">加载失败：{error}<button onclick={load}>重试</button></section>{:else if loading && volumes.length === 0}<section class="state">正在读取 daemon 数据卷…</section>{:else if volumes.length === 0}<section class="state">没有匹配的数据卷</section>{:else}<div class="table"><div class="row head"><span>名称</span><span>驱动</span><span>项目</span><span>路径</span><span>更新时间</span><span>操作</span></div>{#each volumes as item (item.name)}<div class="row"><span class="mono">{volumeName(item)}</span><span>{item.driver || '未提供'}</span><span>{item.projectId || '全局'}</span><span class="mono">{item.path || '未提供'}</span><span>{formatVolumeTime(timestampToIso(item.updatedAt))}</span><span class="actions"><button aria-label={`检查 ${volumeName(item)}`} onclick={() => inspect(item)}>检查</button><button class="danger" aria-label={`删除 ${volumeName(item)}`} onclick={() => remove(item.name, false)} disabled={operating}>删除</button></span></div>{/each}</div>{/if}
