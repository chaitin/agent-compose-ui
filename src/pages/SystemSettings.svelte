<script lang="ts">
  ...
  import TokenManagementPanel from '../components/settings/TokenManagementPanel.svelte';
  ...
  let activeModule = $derived(store.currentPage === 'environment'
    ...
    : store.currentPage === 'tokens' ? 'tokens'
    : 'images');

  const modules: Array<{ id: 'images' | 'environment' | 'capabilities' | 'webhooks' | 'tokens'; label: string; page: Page }> = [
    ...
    { id: 'tokens', label: 'Token 管理', page: 'tokens' },
  ];
...
<div class="system-management">
  ...
  <main class="module-stage">
      ...
      {:else if activeModule === 'tokens'}
        <div id="system-panel-tokens" role="tabpanel" aria-labelledby="system-tab-tokens" class="module-content token-module">
