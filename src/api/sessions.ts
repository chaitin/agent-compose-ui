...
export type WorkSessionCell={id:string;runId?:string;source:string;output:string;type:CellType;exitCode:number;success:boolean;createdAt:string;agent:string;agentSessionId:string;stopReason:string;running:boolean};
...
async function loadWorkSessionHistory(id:string):Promise<WorkSessionHistory>{
  ...
  const firstV2RunAt=runs[0].createdAt;
  ...
  do{
    ...
    for(const item of response.events){
      if(item.kind===RunEventKind.USER_MESSAGE||item.kind===RunEventKind.AGENT_MESSAGE)cells.push({id:item.id,runId:item.runId,source:item.kind===RunEventKind.USER_MESSAGE?item.text:'',output:item.kind===RunEventKind.AGENT_MESSAGE?item.text:'',type:CellType.AGENT,exitCode:item.exitCode,success:item.success,createdAt:timestampString(item.createdAt),agent:item.agent,agentSessionId:'',stopReason:item.stopReason,running:false});
...
async function loadRunsForSandbox(id:string):Promise<SandboxRuns>{
  ...
  return runs.sort((left,right)=>runTimeValue(left.createdAt)-runTimeValue(right.createdAt)||left.runId.localeCompare(right.runId));
}
...
function historyFromSandboxResponse(response:Awaited<ReturnType<typeof sandboxClient.listSandboxHistory>>):WorkSessionHistory{return{cells:response.cells.map((item)=>({id:item.id,source:item.source,output:item.output||[item.stdout,item.stderr].filter(Boolean).join('\n'),type:legacyCellType(item.type),exitCode:item.exitCode,success:item.success,createdAt:timestampString(item.createdAt),agent:item.agent,agentSessionId:item.agentThreadId,stopReason:item.stopReason||'旧会话历史',running:item.running})),events:response.events.map((item)=>({id:item.id,type:item.type,level:item.level,message:item.message,createdAt:timestampString(item.createdAt)}))}}
function isBeforeV2History(createdAt:string,firstV2RunAt:string):boolean{return!createdAt||!firstV2RunAt||runTimeValue(createdAt)<runTimeValue(firstV2RunAt)}
function runTimeValue(value:string):number{const parsed=Date.parse(value);return Number.isNaN(parsed)?0:parsed}
function tagValue(tags:Array<{name:string;value:string}>,name:string):string{return tags.find((tag)=>tag.name.trim()===name)?.value.trim()??''}
