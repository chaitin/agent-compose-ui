...
<PageContent class="space-y-4">
  ...
  <div class="flex flex-wrap gap-2">
    <Input
      bind:value={query}
      class="max-w-sm"
      placeholder={t('过滤引用、标签或 ID')}
      onkeydown={(event: KeyboardEvent) => event.key === 'Enter' && load()}
    /><Button variant="outline" onclick={load} disabled={busy}>{t('查询')}</Button><Input
      bind:value={pullRef}
      class="max-w-sm"
  ...
  {#if busy && !page}<p class="text-sm text-muted-foreground">{t('正在加载镜像…')}</p>{:else if page}
    <div class="text-xs text-muted-foreground">
      Store {t(page.available ? '可用' : '不可用')} · {page.endpoint || t('默认端点')} · {page.totalCount}
      {t('镜像')}
      {page.error}
    </div>
    <div class="overflow-hidden rounded-lg border border-border">
      <table class="w-full text-sm">
        <thead class="bg-muted/40"
          ><tr
            ><th class="p-3 text-left">{t('引用')}</th><th class="p-3 text-left">{t('平台')}</th><th
              class="p-3 text-left">{t('大小')}</th
            ><th class="p-3 text-left">{t('创建时间')}</th><th class="p-3 text-left">{t('容器')}</th><th></th></tr
          ></thead
        ><tbody class="divide-y divide-border"
          >{#each page.images as item (item.imageId + item.imageRef)}<tr
              ...
              ><td class="p-3">{item.containerCount.toString()}</td><td class="p-3 text-right"
                ><Button variant="ghost" size="sm" onclick={() => inspect(item)}>{t('检查')}</Button><Button
                  variant="ghost"
                  size="sm"
                ...
                ></td
              ></tr
            >{:else}<tr><td colspan="6" class="p-10 text-center text-muted-foreground">{t('没有镜像')}</td></tr
            >{/each}</tbody
        >
      </table>
    </div>
  ...
  {#if selected}<div class="rounded-lg border border-border bg-card p-4">
      <div class="mb-2 flex justify-between">
        <h2 class="font-medium">{t('镜像详情')}</h2>
        <button onclick={() => (selected = null)}>{t('关闭')}</button>
      </div>
      <dl class="mb-4 grid grid-cols-[6rem_minmax(0,1fr)] gap-2 text-sm">
        ...
        <dt class="text-muted-foreground">{t('引用')}</dt>
        ...
        <dt class="text-muted-foreground">{t('检查时间')}</dt>
