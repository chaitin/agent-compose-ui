<script lang="ts">
  ...
  async function saveEnv(): Promise<void> {
    await perform('全局环境已保存', () => updateEnvItems(envItems));
  }
  async function saveGateway(): Promise<void> {
    await perform('能力网关已保存', async () => {
  ...
  async function testGateway(): Promise<void> {
    await perform('能力网关状态已刷新', async () => {
      gatewayStatus = await getCapabilityStatus();
      if (!gatewayStatus.ok) throw new Error(gatewayStatus.error || gatewayStatus.status || '能力网关连接失败');
    });
  }
  ...
  async function createWorkspace(): Promise<void> {
    try {
      ...
      error = 'Workspace 配置必须是有效 JSON';
      return;
    }
    await perform('Workspace 预设已创建', async () => {
      const input = {
        name: workspaceName,
        type: workspaceType,
  ...
  async function removeWorkspace(item: WorkspacePreset): Promise<void> {
    ...
    await perform('Workspace 预设已删除', async () => {
  ...
  async function createWebhook(): Promise<void> {
    if (!webhookId.trim() || !webhookName.trim()) {
      error = 'Webhook ID 与名称必填';
      return;
    }
    await perform('Webhook 来源已保存', async () => {
  ...
  async function removeWebhook(item: WebhookSource): Promise<void> {
    ...
    await perform('Webhook 来源已删除', async () => {
  ...
  <PageContent class="flex min-h-0 flex-1 flex-col overflow-hidden">
    {#if error}<div class="mb-4 shrink-0 rounded-md bg-destructive/10 p-3 text-sm text-destructive">
      ...
      ><Tabs.List data-scroll-surface class="shrink-0 max-w-full justify-start overflow-x-auto"
        ><Tabs.Trigger value="env">全局环境</Tabs.Trigger><Tabs.Trigger value="gateway">能力网关</Tabs.Trigger
        ><Tabs.Trigger value="webhook">Webhook</Tabs.Trigger><Tabs.Trigger
          value="workspace"
          onpointerenter={preloadMonaco}
          onfocus={preloadMonaco}>Workspace 预设</Tabs.Trigger
        ><Tabs.Trigger value="tokens">API Token</Tabs.Trigger><Tabs.Trigger value="auth">鉴权</Tabs.Trigger></Tabs.List
      >
      <Tabs.Content data-scroll-pane value="env" class="mt-4 min-h-0 max-w-3xl flex-1 space-y-3 overflow-y-auto pr-1"
        >{#each envItems as item, index (index)}<div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_auto_auto]">
            ...
            /><Input
              ...
              placeholder={item.secret && !item.valueKnown ? '未修改' : 'value'}
            /><label class="flex items-center gap-1 text-xs"
              ...
              />Secret</label
            ><Button variant="ghost" size="sm" onclick={() => removeEnv(index)}>移除</Button>
          </div>{/each}
        <div class="flex gap-2">
          <Button variant="outline" size="sm" onclick={addEnv}>添加变量</Button><Button
            size="sm"
            disabled={busy}
            onclick={saveEnv}>保存</Button
          >
        </div></Tabs.Content
      >
      ...
      <Tabs.Content
        ...
        ><div class="rounded-lg border border-border bg-card p-4 text-sm">
          <div class="flex items-center gap-2">
            ...
            <span>{gatewayStatus?.ok ? 'OctoBus 已连接' : 'OctoBus 未连接'}</span>
            <span class="text-muted-foreground">· {gatewayStatus?.serviceCount ?? 0} 个服务</span>
          </div>
          {#if gatewayStatus?.error}<p class="mt-2 text-destructive">{gatewayStatus.error}</p>{/if}
        </div>
        <label class="block space-y-1"><span class="text-sm">网关地址</span><Input bind:value={gatewayAddr} /></label
        ><label class="block space-y-1"
          ><span class="text-sm">访问 Token</span><Input
            bind:value={gatewayToken}
            type="password"
            placeholder={gatewayTokenSet ? '已设置；留空保持不变' : '可选'}
          /></label
        >{#if gatewayTokenSet}<label class="flex items-center gap-2 text-sm"
            ><input type="checkbox" bind:checked={gatewayClearToken} />清除已保存 Token</label
          >{/if}
        <div class="flex gap-2">
          <Button onclick={saveGateway} disabled={busy}>保存网关</Button><Button
            variant="outline"
            onclick={testGateway}
            disabled={busy}>测试连接</Button
          >
        </div></Tabs.Content
      >
      <Tabs.Content data-scroll-pane value="webhook" class="mt-4 min-h-0 flex-1 space-y-4 overflow-y-auto pr-1"
        ><div class="flex items-center justify-between">
          <div>
            <h2 class="text-sm font-medium">Webhook 来源</h2>
            <p class="text-xs text-muted-foreground">配置入口鉴权后，可在事件中心查询真实投递和关联运行。</p>
          </div>
          <Button variant="outline" size="sm" onclick={() => navigate('/events')}>打开事件中心</Button>
        </div>
        <form
        ...
        >
          <Input bind:value={webhookId} placeholder="唯一 ID" disabled={editingWebhook} /><Input
            bind:value={webhookName}
            placeholder="名称"
          /><Input bind:value={webhookProvider} placeholder="Provider" /><Input
            bind:value={webhookTopic}
            placeholder="Topic 前缀"
          /><Input bind:value={webhookToken} type="password" placeholder="访问 Token（留空保持）" /><Input
            bind:value={webhookSignatureType}
            placeholder="签名类型，如 hmac-sha256"
          /><Input bind:value={webhookSignatureSecret} type="password" placeholder="签名密钥（留空保持）" /><label
            class="flex items-center gap-2 text-sm"
            ><span>Body 上限</span><Input bind:value={webhookBodyLimit} type="number" /></label
          ><label class="flex items-center gap-2 text-sm"
            ><input type="checkbox" bind:checked={webhookEnabled} />启用</label
          >
          <div class="flex gap-2">
            <Button type="submit">{editingWebhook ? '保存修改' : '添加来源'}</Button>{#if editingWebhook}<Button
                type="button"
                variant="outline"
                onclick={resetWebhookDraft}>取消</Button
              >{/if}
          </div>
        </form>
          ...
          >
            <div>
              ...
              <div class="text-xs text-muted-foreground">
                {item.provider} · {item.topicPrefix} · {item.enabled ? '启用' : '停用'}
              </div>
            </div>
            <div class="flex">
              <Button variant="ghost" size="sm" onclick={() => editWebhook(item)}>编辑</Button><Button
                variant="ghost"
                size="sm"
                class="text-destructive"
                onclick={() => removeWebhook(item)}>删除</Button
              >
            </div>
          </div>{:else}<p class="text-sm text-muted-foreground">没有 Webhook 来源</p>{/each}</Tabs.Content
      >
        ...
        >
          ...
          >
            <div class="grid gap-2 md:grid-cols-[1fr_12rem_2fr]">
              <Input bind:value={workspaceName} placeholder="名称" /><select
            ...
            <div class="min-h-[20rem] lg:min-h-0 lg:flex-1">
              <CodeEditor bind:value={workspaceConfig} language="json" height="100%" title="Workspace 配置 JSON" />
            </div>
            <div class="flex shrink-0 gap-2">
              <Button type="submit">{editingWorkspaceId ? '保存修改' : '创建'}</Button>{#if editingWorkspaceId}<Button
                  type="button"
                  variant="outline"
                  onclick={resetWorkspaceDraft}>取消编辑</Button
                >{/if}
            </div>
          </form>
          <aside data-scroll-pane class="space-y-3 lg:min-h-0 lg:overflow-y-auto lg:pr-1">
            <div class="flex items-center justify-between">
              <div>
                <h2 class="text-sm font-medium">已有预设</h2>
                <p class="text-xs text-muted-foreground">{workspaces.length} 个 Workspace</p>
              </div>
              {#if editingWorkspaceId}<Button variant="outline" size="sm" onclick={resetWorkspaceDraft}>新建</Button
                >{/if}
            </div>
            {#each workspaces as item (item.id)}<div
              ...
              >
                ...
                <div class="flex">
                  <Button variant="ghost" size="sm" onclick={() => editWorkspace(item)}>编辑</Button><Button
                    variant="ghost"
                    size="sm"
                    class="text-destructive"
                    onclick={() => removeWorkspace(item)}>删除</Button
                  >
                </div>
              </div>{:else}<p class="text-sm text-muted-foreground">没有 Workspace 预设</p>{/each}
          </aside>
        </div></Tabs.Content
      >
      <Tabs.Content
        ...
        ><dl class="grid grid-cols-[8rem_1fr] gap-2">
          <dt class="text-muted-foreground">认证</dt>
          <dd>{auth?.enabled ? '已启用' : '未启用'}</dd>
          <dt class="text-muted-foreground">当前用户</dt>
          <dd>{auth?.user?.displayName || auth?.username || '匿名'}</dd>
          ...
          <dd>{auth?.oauthEnabled ? '已启用' : '未启用'}</dd>
