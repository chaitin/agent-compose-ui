...
服务端全局变量引用需要同时设置 `AGENT_COMPOSE_DB_PATH`（daemon SQLite，只读）和 `UI_STATE_DB_PATH`（UI shadow SQLite，可写）。配置后，浏览器只保存和发送 `${VAR}` 引用，网关在 Validate/Apply 时读取 daemon 的 `global_env` 并解析；字面量保持不变。修改全局变量只标记相关项目待同步，绝不会自动 Apply、运行或改变调度，直到用户明确保存或启用项目。daemon 仍会保存解析后的明文，因此 daemon 数据目录仍属于敏感数据。Docker full 模式已默认按此边界配置，详情见 [`docker/README.md`](docker/README.md)。
